        ...
        <header>
            ...
            <div class="header_c">
                <form id="index_submit">
                    <div class="header_c_div"><span id="header_title">跟团游</span><img src="/harmony/images/search_select_sj.png"></div>
            ...
            <form>
                ...
                <div class="selectCondition_list clearboth">
                    ...
                    <ul class="selectCondition_r celect_radio" id="confirmType">
                        <li><a href="javascript:void(0)" data-type="INSTANT">即时确定</a> </li>
...
$(function(){
    if (getUrlParam('title')) {
        $("#value").val(decodeURI(escape(getUrlParam('title'))))
    };
    var header_title = getUrlParam('regimentType');
    if (header_title == 'regiment') {
        $("#productlist").text('跟团游')
        $("#header_title").text('跟团游')
    }else if (header_title == 'free') {
        $("#productlist").text('自由行')
        $("#header_title").text('自由行')
    }else if (header_title == 'half') {
        $("#productlist").text('半自助')
        $("#header_title").text('半自助')
    };
})
    var hotel = true;
    var sup = true;
    var vm = new Vue({
        ...
        methods:{
            ...
            listpost:function(p){
                ...
                //产品类型
                var regimentType = '';
                if(getUrlParam('regimentType')){
                    regimentType = getUrlParam('regimentType')
                }
                //升降序
                ...
                var title = '';
                if(getUrlParam('title')){
                    title = decodeURI(escape(getUrlParam('title')))
                }
                ...
                this.$http.post('/harmony/listpost',{
                    ...
                    if(response.body != ''){
                        ...
                        this.target = eval(response.body.target);
...
$("#index_submit").submit(function(){
    var value = $("#value").val();
    if (!key) {
        key = getUrlParam('regimentType');
    };
    var url_val = '?regimentType='+key
    if (value) {
        url_val += '&title='+value
    };
